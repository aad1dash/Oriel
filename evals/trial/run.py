...
MCP_CONFIG = ROOT / "evals/trial/mcp.json"
DEFAULT_REPO = Path("/path/to/an/unrelated/repository")
...
ALLOWED = ["mcp__oriel__search_source", "mcp__oriel__read_source", "Read", "Grep", "Glob"]
DENIED = ["Edit", "Write", "NotebookEdit", "Bash", "WebFetch", "WebSearch", "Task", "Agent"]


def ask(question: dict, repo: Path) -> dict:
...
def summarise(question: dict, stdout: str, stderr: str) -> dict:
...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=arguments.workers) as pool:
        results = list(pool.map(lambda q: ask(q, arguments.repo), questions))

    (ROOT / "evals/trial/results.json").write_text(json.dumps(results, indent=2))
    for result in results:
        matched = "ok " if result["first_oriel_tool"] == result["expected_tool"] else "OFF"
        print(
